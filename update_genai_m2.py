...
# New Module 2 Data (Rich Content)
new_module_content = """
            {
...
def update_module_2():
    try:
        ...
        # 3. WE NEED THE SECOND MODULE BLOCK (Index 1)
        # We need to skip the first module.
        # Strategy: Count braces to find where Module 1 ends, then look for start of Module 2.
        
        brace_level = 0
        module_1_found = False
        module_1_end_idx = -1
        
        # Start searching after "modules: ["
        current_idx = modules_start_idx + 1
        
        # Find start of Module 1 (first `{` at correct level)
        # We assume structure is valid: modules: [ { ... }, { ... } ]
        
        # Iterate to find the end of the first module
        for i in range(current_idx, len(lines)):
            line = lines[i]
            
            # Simple brace counting logic for top-level objects in the array
            # This relies on the file being formatted reasonably well (one module object per block)
            
            if not module_1_found:
                if "{" in line:
                    module_1_found = True
        ...
        if module_1_end_idx == -1:
            print("❌ Could not find end of Module 1.")
            return
            
        print(f"✅ Module 1 ends at line {module_1_end_idx + 1}")
        
        # 4. Find start of Module 2
        module_2_start_idx = -1
        for i in range(module_1_end_idx + 1, len(lines)):
            if "{" in lines[i]:
                module_2_start_idx = i
                break
        
        if module_2_start_idx == -1:
             print("❌ Could not find start of Module 2.")
             return

        print(f"✅ Found Module 2 Start at line {module_2_start_idx + 1}")

        # 5. Find end of Module 2
        brace_level = 0
        module_2_end_idx = -1
        
        for i in range(module_2_start_idx, len(lines)):
            line = lines[i]
            brace_level += line.count('{')
            brace_level -= line.count('}')
        ...
        if module_2_end_idx == -1:
             print("❌ Could not find end of Module 2.")
             return

        print(f"✅ Found Module 2 End at line {module_2_end_idx + 1}")
        
        # 6. Replace Module 2 content
        # Check for comma after Module 2 (if there's a Module 3)
        has_comma = lines[module_2_end_idx].strip().endswith(',')
        
        new_lines = lines[:module_2_start_idx]
        new_lines.append(new_module_content.strip() + ("," if has_comma else "") + "\n")
        new_lines.extend(lines[module_2_end_idx+1:])
        
        with open(file_path, 'w', encoding='utf-8') as f:
            f.writelines(new_lines)
            
        print("✅ Successfully updated GenAI Module 2!")
