...
Ans: A) 210 (1100 -1000 wait 1210-1000=210).
P(n,r) for n=5 r=3?
...
# Very simple manual parser to convert into a TS list of objects
blocks = raw_text.split("\n\n")

questions = []
current_category = "Quantitative Aptitude"
id_counter = 1
...
for block in blocks:
    ...
    # Category markers
    if "Logical Reasoning" in lines[0] or "Number Series" in lines[0] or "Coding-Decoding" in lines[0] or "Blood Relations" in lines[0] or "Syllogism" in lines[0]:
        current_category = "Logical Reasoning"
    elif "Verbal Ability" in lines[0] or "Sentence / Word" in lines[0] or "Sentence Completion" in lines[0] or "Error Identification" in lines[0] or "Meanings" in lines[0]:
        current_category = "Verbal Ability"
    elif "HCF & LCM" in lines[0]:
        current_category = "Quantitative Aptitude"
        
    # Is it a question block? usually has Ans: and A) B) C) D)
    q_line = ""
    opts = []
    ans = ""
    logic = ""
    ...
    in_q = False
    for L in lines:
        if L.startswith("a)") or L.startswith("A)") or L.startswith("b)") or L.startswith("B)") or (" A) " in L and " B) " in L):
            # parse options
            opt_str = L
            # simplistic parsing
            o1 = re.search(r'[aA]\)\s*(.*?)(?=\s*[bB]\)|\Z)', opt_str)
            o2 = re.search(r'[bB]\)\s*(.*?)(?=\s*[cC]\)|\Z)', opt_str)
            o3 = re.search(r'[cC]\)\s*(.*?)(?=\s*[dD]\)|\Z)', opt_str)
            o4 = re.search(r'[dD]\)\s*(.*?)(?=\Z)', opt_str)
            
            if o1: opts.append(o1.group(1).strip())
            if o2: opts.append(o2.group(1).strip())
            if o3: opts.append(o3.group(1).strip())
            if o4: opts.append(o4.group(1).strip())
                
        elif L.startswith("Ans:"):
            # extract answer and possibly logic
            match = re.search(r'Ans:\s*[a-dA-D]\)\s*(.*?)(?:\s*\((.*?)\))?$', L)
            if match:
               ...
               # maybe just "Ans: A. 30."
               match2 = re.search(r'Ans:\s*[a-dA-D]\)?\s*(.*)', L)
               if match2:
                  ans_text = match2.group(1).strip()
                  # maybe logic is in next line if it starts with (
                  if '(' in ans_text:
                      parts = ans_text.split('(')
                      ans = parts[0].strip()
                      logic = parts[1].replace(')','').strip()
                  else:
                      ans = ans_text
        elif L.startswith("("):
            logic += " " + L.replace('(','').replace(')','')
        elif not L.startswith("1-") and not L.startswith("11-") and not L.startswith("21-") and not L.startswith("31-") and not L.startswith("41-"):
             # probably the question
             q_line += " " + L

    if q_line and opts and ans:
        # Construct object
        questions.append({
            "id": f"q{id_counter}",
            "category": current_category,
            "question": q_line.strip(),
            "options": [opts[0] if len(opts) > 0 else "A", opts[1] if len(opts) > 1 else "B", opts[2] if len(opts) > 2 else "C", opts[3] if len(opts) > 3 else "D"],
            "correctAnswer": ans,
            "explanation": logic.strip() if logic else "Standard application of formulas."
        })
        id_counter += 1
