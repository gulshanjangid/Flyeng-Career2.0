...
import { FileText, Cpu, User, Globe, Rocket, X, Target, Zap, ChevronRight, Activity } from "lucide-react"
...
type NodeType = keyof typeof DEMO_DATA;

/* --- MAIN COMPONENT --- */

export function SystemDemoDialog({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        }, 4000); // Slower, 4s cycle for elegance
            ...
            <DialogContent showCloseButton={false} className="max-w-none w-[95vw] lg:w-[90vw] lg:max-w-6xl h-[85vh] sm:h-[80vh] bg-zinc-950 text-white p-0 overflow-hidden shadow-2xl border border-white/5 outline-none rounded-3xl z-[100] flex flex-col items-center justify-center">
                ...
                <div className="flex-1 w-full relative flex items-center justify-center overflow-hidden bg-zinc-950 font-sans">
                    ...
                    <div className="w-full h-full relative flex items-center justify-center p-8 z-10">
                        ...
                        {/* Desktop: Radial Diagram - Clean & Balanced */}
                        <div className="hidden lg:block relative w-full h-full max-w-4xl max-h-[500px] shrink-0">
                            {/* SVG Connections - Smooth Curves */}
                            <svg className="absolute inset-0 w-full h-full z-0 pointer-events-none">
                                {/* Ecosystem Ring */}
                                <circle cx="50%" cy="50%" r="35%" fill="none" stroke="white" strokeWidth="1" strokeOpacity="0.05" strokeDasharray="4 4" />
                                
                                <ConnectionPath x1="50%" y1="50%" x2="25%" y2="25%" color="#6366f1" active={isActive('architect')} label="Extract" />
                                <ConnectionPath x1="50%" y1="50%" x2="75%" y2="25%" color="#8b5cf6" active={isActive('analyze')} label="Analyze" />
                                <ConnectionPath x1="50%" y1="50%" x2="25%" y2="75%" color="#ec4899" active={isActive('match')} label="Match" />
                                <ConnectionPath x1="50%" y1="50%" x2="75%" y2="75%" color="#10b981" active={isActive('prepare')} label="Prepare" />
                            </svg>
                            ...
                            <div className="absolute top-[25%] left-[25%] -translate-x-1/2 -translate-y-1/2 z-20">
                                <DesktopNode id="architect" data={DEMO_DATA.architect} active={isActive('architect')} />
                            </div>
                            <div className="absolute top-[25%] right-[25%] translate-x-1/2 -translate-y-1/2 z-20">
                                <DesktopNode id="analyze" data={DEMO_DATA.analyze} active={isActive('analyze')} />
                            </div>
                            <div className="absolute bottom-[25%] left-[25%] -translate-x-1/2 translate-y-1/2 z-20">
                                <DesktopNode id="match" data={DEMO_DATA.match} active={isActive('match')} />
                            </div>
                            <div className="absolute bottom-[25%] right-[25%] translate-x-1/2 translate-y-1/2 z-20">
...
function MobileNode({ id, data, active, isCore = false }: any) {
    ...
    return (
        <div className={cn(
            ...
            active 
                ? `border-${data.color}-500/30 bg-${data.color}-500/5` 
...
function DesktopNode({ id, data, active, isCore = false }: any) {
    ...
    return (
        <motion.div
            ...
            className={cn(
                "relative flex flex-col items-center justify-center border transition-colors duration-1000 backdrop-blur-2xl shadow-2xl",
                // Elegant Shape: Circle for Core, Soft Square for others
                isCore ? "rounded-full aspect-square border-white/10 bg-white/5" : "rounded-3xl aspect-square border-white/5 bg-white/5", 
                size,
                active ? `border-${data.color}-500/30 bg-white/10` : ""
            )}
        >
...
function ConnectionPath({ x1, y1, x2, y2, color, active, label }: any) {
    return (
        <g>
            ...
            {/* Active Path - Smooth Draw */}
            <motion.line
                x1={x1} y1={y1} x2={x2} y2={y2}
                stroke={active ? color : "transparent"}
                strokeWidth={active ? 1.5 : 0} // Thin, elegant line
                initial={{ pathLength: 0, opacity: 0 }}
                animate={{ pathLength: active ? 1 : 0, opacity: active ? 0.6 : 0 }} // Lower opacity for elegance
                transition={{ duration: 1.2, ease: "easeInOut" }}
            />

            {/* Floating Particle - Slow & Smooth */}
            {active && (
                <motion.circle
                    r="2" // Smaller particle
                    fill={color}
